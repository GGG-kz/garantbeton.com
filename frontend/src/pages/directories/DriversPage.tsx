...
export default function DriversPage() {
  ...
  // Функция для фильтрации водителей
  const handleSearch = (search: string) => {
    const filtered = drivers.filter(driver => {
      const matchesSearch = driver.fullName.toLowerCase().includes(search.toLowerCase()) ||
                           driver.phone.toLowerCase().includes(search.toLowerCase()) ||
                           driver.login.toLowerCase().includes(search.toLowerCase())
  ...
  // Обновляем фильтрацию при изменении водителей
  useEffect(() => {
  ...
  const handleSave = async (data: CreateDriverRequest) => {
    try {
      if (editingDriver) {
        const updatedDriver = await driversApi.update(editingDriver.id, data)
      ...
      setError('Ошибка сохранения водителя')
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-900">Водители</h2>
              <p className="text-gray-600">Всего: {filteredDrivers.length}</p>
            </div>
          </div>
          <button 
            onClick={handleAdd}
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {['active', 'vacation', 'sick', 'inactive'].map((status) => (
                <button
                  key={status}
                  onClick={() => {
                    setFilterStatus(status as Driver['status'])
                    handleSearch(searchTerm)
                  }}
                  className={`px-4 py-2 rounded-lg transition-colors duration-200 ${
                    filterStatus === status
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredDrivers.map((driver) => {
            return (
              <div key={driver.id} className="bg-white rounded-lg border border-gray-200 p-6 hover:shadow-lg transition-shadow duration-200">
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-gray-50 p-3 rounded-lg">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm text-gray-600">Временный пароль:</span>
                      </div>
                  ...
                  {driver.vehicleIds.length > 0 && (
                    <div>
                      <p className="text-sm font-medium text-gray-700 mb-2">Закрепленный транспорт:</p>
                      <div className="flex flex-wrap gap-1">
                        {driver.vehicleIds.map((vehicleId, index) => (
                          <span key={index} className="inline-flex items-center space-x-1 px-2 py-1 text-xs bg-blue-100 text-blue-800 rounded">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-900">{viewingDriver.fullName}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">Статус</label>
                <p className="mt-1 text-sm text-gray-900">{getStatusLabel(viewingDriver.status)}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-900">{viewingDriver.phone}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">Логин</label>
                <p className="mt-1 text-sm font-mono text-gray-900">{viewingDriver.login}</p>
              </div>
            </div>

            <div className="grid grid-cols-1 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700">Статус пароля</label>
