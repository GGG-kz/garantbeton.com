...
export default function ExpenseInvoiceForm({
  ...
  // Принудительное обновление v8 - добавлено отслеживание выполнения заказов
  console.log('ExpenseInvoiceForm v8 - order completion tracking added')
  ...
  // Функция для обновления заказа после создания накладной
  const updateOrderWithInvoiceInfo = (invoiceId: string) => {
    if (!formData.orderId) return

    const updatedOrders = orders.map(order => {
      if (order.id === formData.orderId) {
        return {
          ...order,
          assignedDriverId: formData.delivery.driverId,
          assignedDriverName: formData.delivery.driverName,
          assignedVehicleId: formData.delivery.vehicleId,
          assignedVehicleNumber: formData.delivery.vehicleNumber,
          departureTime: formData.timing.departureFromPlant,
    ...
    // Обновляем заказы в localStorage
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      // Сначала сохраняем накладную
      onSave(formData)
      
      // Затем обновляем заказ (если он связан с накладной)
      if (formData.orderId) {
        // Генерируем временный ID для накладной (в реальном приложении это будет делать бэкенд)
        const tempInvoiceId = `invoice_${Date.now()}`
        updateOrderWithInvoiceInfo(tempInvoiceId)
      }
    }
  }
  ...
  return (
    <div className="max-w-6xl mx-auto bg-white rounded-lg shadow-lg">
      <div className="p-6 border-b border-mono-200">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold text-mono-900">
            {isEditing ? 'Редактировать расходную накладную' : 'Создать расходную накладную'}
          </h2>
        </div>
        ...
        {!isEditing && (
          <div className="bg-mono-50 border border-mono-200 rounded-lg p-4">
            ...
            <div className="mb-3 p-2 bg-yellow-100 border border-yellow-300 rounded text-sm">
              ...
              <p>Подходящие заказы: {orders.filter(order => order.status === 'confirmed' || order.status === 'in_production').length}</p>
              <p>Статусы заказов: {orders.map(o => o.status).join(', ')}</p>
            </div>
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-8">
        ...
        {/* Расходы материалов (только для внутреннего учета) */}
        <div className="bg-mono-50 p-4 rounded-lg">
          <h3 className="text-lg font-semibold text-black mb-4">Расходы материалов (внутренний учет)</h3>
          <div className="space-y-4">
            {formData.items.map((item, index) => (
              <div key={item.id} className="bg-white p-4 rounded border">
                <h4 className="font-medium text-sm text-mono-700 mb-3">{item.productName || 'Марка бетона'}</h4>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div>
                    <label className="block text-xs text-mono-600 mb-1">Цемент (кг/м³)</label>
                    <input
                      type="number"
                      value={item.cementConsumption || ''}
                      onChange={(e) => handleItemChange(item.id, 'cementConsumption', parseFloat(e.target.value) || 0)}
                      className="w-full px-3 py-2 text-sm border border-mono-300 rounded focus:ring-1 focus:ring-mono-500 focus:border-mono-500"
                      placeholder="0"
                    />
                  </div>
                  <div>
                    <label className="block text-xs text-mono-600 mb-1">Щебень (кг/м³)</label>
                    <input
                      type="number"
                      value={item.gravelConsumption || ''}
                      onChange={(e) => handleItemChange(item.id, 'gravelConsumption', parseFloat(e.target.value) || 0)}
                      className="w-full px-3 py-2 text-sm border border-mono-300 rounded focus:ring-1 focus:ring-mono-500 focus:border-mono-500"
                      placeholder="0"
                    />
                  </div>
                  <div>
                    <label className="block text-xs text-mono-600 mb-1">Песок (кг/м³)</label>
                    <input
                      type="number"
                      value={item.sandConsumption || ''}
                      onChange={(e) => handleItemChange(item.id, 'sandConsumption', parseFloat(e.target.value) || 0)}
                      className="w-full px-3 py-2 text-sm border border-mono-300 rounded focus:ring-1 focus:ring-mono-500 focus:border-mono-500"
                      placeholder="0"
                    />
                  </div>
                  <div>
                    <label className="block text-xs text-mono-600 mb-1">Пластификатор (л/м³)</label>
                    <input
                      type="number"
                      step="0.1"
                      value={item.plasticizerConsumption || ''}
                      onChange={(e) => handleItemChange(item.id, 'plasticizerConsumption', parseFloat(e.target.value) || 0)}
                      className="w-full px-3 py-2 text-sm border border-mono-300 rounded focus:ring-1 focus:ring-mono-500 focus:border-mono-500"
                      placeholder="0"
                    />
                  </div>
          ...
          <p className="text-xs text-black mt-3">
            * Эти поля автоматически заполняются при выборе марки бетона и используются для внутреннего учета запасов
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-mono-700 mb-1">Вес нетто (т)</label>
                <input
                  type="number"
                  step="0.01"
                  value={formData.delivery.netWeight || ''}
                  onChange={(e) => handleDeliveryChange('netWeight', parseFloat(e.target.value) || undefined)}
                  className="w-full px-3 py-2 border border-mono-300 rounded-md"
                />
              </div>
              
              <div>
                <label className="block text-sm font-medium text-mono-700 mb-1">Вес брутто (т)</label>
                <input
                  type="number"
                  step="0.01"
                  value={formData.delivery.grossWeight || ''}
                  onChange={(e) => handleDeliveryChange('grossWeight', parseFloat(e.target.value) || undefined)}
                  className="w-full px-3 py-2 border border-mono-300 rounded-md"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-mono-700 mb-1">Осадка конуса</label>
                <input
                  type="number"
                  step="0.1"
                  value={formData.delivery.coneSlump || ''}
                  onChange={(e) => handleDeliveryChange('coneSlump', parseFloat(e.target.value) || undefined)}
                  className="w-full px-3 py-2 border border-mono-300 rounded-md"
                />
              </div>
              
              <div>
                <label className="block text-sm font-medium text-mono-700 mb-1">Объём с нач. дня (м³)</label>
                <input
                  type="number"
                  step="0.1"
                  value={formData.delivery.dailyVolume || ''}
                  onChange={(e) => handleDeliveryChange('dailyVolume', parseFloat(e.target.value) || undefined)}
                  className="w-full px-3 py-2 border border-mono-300 rounded-md"
                />
              </div>
            </div>
